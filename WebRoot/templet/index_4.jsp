                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade" id="meg3-tab03">
                            <ul class="msg-list" id="block7">
                                <li><span class="time">2016-11-11</span><a>《全国消防技术服务规章制度》</a></li>
...
<script>
    $(function(){
       //菜单点击样式
       $('.head .menu a#menu04').addClass('active');
      ...
      $.postJSON("<%=request.getContextPath()%>/templetController/getChildColumn",con,function(data){
      	if(data){
      		...
      		if(data.length>=5){
          		...
          		$("#block5_url").html("href","list.jsp?article_type_id="+data[0].ARTICLE_TYPE_ID);
				getBlock5(data[4].ARTICLE_TYPE_ID);
      		}
      		if(data.length>=6){
          		$("#block6_name").val(data[5].ARTICLE_TYPE_NAME);
          		$("#block6_url").html("href","list.jsp?article_type_id="+data[0].ARTICLE_TYPE_ID);
				getBlock6(data[5].ARTICLE_TYPE_ID);
      		}
      		if(data.length>=7){
          		$("#block7_name").val(data[6].ARTICLE_TYPE_NAME);
          		$("#block7_url").html("href","list.jsp?article_type_id="+data[0].ARTICLE_TYPE_ID);
